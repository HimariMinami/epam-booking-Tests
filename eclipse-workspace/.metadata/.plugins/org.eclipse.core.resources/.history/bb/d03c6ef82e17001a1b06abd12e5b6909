...
public class BookingAccomodationsPage {
	...
	public BookingAccomodationPage accomodationDescription(WebDriver driver, String place, int num) throws InterruptedException
	{
	...
	{
		Assert.assertTrue(cityNumIsCorrect(driver, place, num), "The number of accomodation for the " + place + " is not correct!");
	...
	public boolean cityNumIsCorrect(WebDriver driver, String place, int num)
	{
		try
		{
			List<WebElement> elements = driver.findElements(By.xpath("//a[@class=\"bui-link\" and contains(text(), \""+ place + "\")]"));
